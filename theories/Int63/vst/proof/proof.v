...
Module Facts.
    ...
    Proof.
        ...
        {
            apply encode_Z.tight_bound in Hx.
            unfold encode_Z in Hx.
            assert (H': Z.pos x' * 2 - 1 < - Int64.min_signed). { lia. }
            rename H' into Hx_l.

            assert (H': Z.pos x' * 2 - 1 <= - Int64.min_signed - 1). { lia. }
            clear Hx_l. rename H' into Hx_l.

            apply Z.log2_le_mono in Hx_l.

            assert (E: Int64.zwordsize - 1 - 1 = 62). { reflexivity. } rewrite E in * ; clear E.
            assert (E: Int64.zwordsize - 1 = 63). { reflexivity. } rewrite E in * ; clear E.
            assert (E: Z.log2 (- Int64.min_signed - 1) = 62). { reflexivity. } rewrite E in * ; clear E.
            ...
            {
                assert (E: Int64.min_signed = - ((two_power_nat 62) * 2)). { reflexivity. } rewrite E in * ; clear E.
                lia.
            }
            ...
            destruct Hx as [ Hx | Hx ].
            {
                ...
                assert (E: Z.log2 (two_power_nat 62 - 1) = 61). { reflexivity. } rewrite E in * ; clear E.
                lia.
            }
            {
                inversion Hx.
...
Proof.
    ...
    {
        apply encode_Z.tight_bound in H0.
        unfold encode_Z in *.
        lia.
    }
    simpl.
...
Proof.
    ...
    {
        apply encode_Z.tight_bound in H1.
        lia.
    }
    repeat step ; try entailer!.
    {
...
Proof.
    start_function.
    repeat step ; try entailer!.
    repeat rewrite Int64.signed_repr ; apply encode_Z.bound ; assumption.
Qed.
...
Proof.
    ...
    repeat step ; try entailer!.
    {
        constructor.
        {
            ...
            {
                rewrite Int64.signed_repr ; try apply encode_Z.bound ; trivial.
            }
            {
                rewrite F ; trivial.
            }
        }
        {
            ...
            rewrite Int64.signed_repr in E ; try apply encode_Z.bound ; try lia.
    ...
    {
        repeat f_equal ; try rewrite Int64.signed_repr ; try apply encode_Z.bound ; trivial.
    }
Qed.
...
Proof.
    ...
    {
        repeat f_equal ; rewrite Int64.signed_repr ; try apply encode_Z.bound ; trivial.
    }
Qed.
...
Proof.
    ...
    repeat step ; try entailer!.
    repeat f_equal.
    repeat rewrite encode_Z.bitwise.
